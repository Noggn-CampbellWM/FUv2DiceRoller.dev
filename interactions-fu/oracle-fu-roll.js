async function runSlashRoller (options,interaction) {
  const modDiceAmmt = options.getNumber('dice_number') || 0;
  const modDiceType = options.getNumber('dice_type')-1 || 19;
  const modDiceTypeBase = options.getNumber('dice_type') || 20;
  const modDiceMod = options.getNumber('dice_mod') || 0;
  const modRollType = options.getString('roll_type') || "total";
  const modExplodeType = options.getNumber('exploding_dice') || 0;
